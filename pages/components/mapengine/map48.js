...
import { register_24048 } from "../../../lib/registerproj4";
...
export default function Map48(props) {
    ...
    const _zoomtoWkt = (wkt) => {
        register_24048()
        let f = new WKT().readFeature(wkt, { dataProjection: "EPSG:24048", featureProjection: "EPSG:4326" })
        var ext = f.getGeometry().getExtent();
        map.getView().fit(ext, { duration: 500 });
    }
    const _zoomtoWktHightlight = (wkt) => {
        register_24048()
        let features = new WKT().readFeature(wkt, { dataProjection: "EPSG:24048", featureProjection: "EPSG:4326" })
    ...
    const _zoomtoWktArray = (wkt_array) => {
        ...
        for (let i in wkt_array) {
            let f = new WKT().readFeature(wkt_array[i], { dataProjection: "EPSG:24048", featureProjection: "EPSG:4326" });
            newArray.push(f);
        }
        map.getLayers().forEach(function (layer) {
            if (layer) {
    ...
    const _zoomtoPointForScreenshot = (point, showmarker = true) => {
        ...
        register_24048();
        let f = new WKT().readFeature(point, { dataProjection: "EPSG:24048", featureProjection: "EPSG:4326" })
        let lonlat = f.values_.geometry.flatCoordinates;
        var lon = lonlat[0];
        var lat = lonlat[1];
        let view = map.getView();
    ...
    const _zoomtoPointBillboardArrayForScreenshot = (pointArray) => {
        ...
        let lonlatX = [];
        for (let i in pointArray) {
            let f = new WKT().readFeature(pointArray[i], { dataProjection: "EPSG:24048", featureProjection: "EPSG:4326" })
            // let lonlat = f.values_.geometry.flatCoordinates;
            // var lon = lonlat[1];
            // var lat = lonlat[0];
