...
export default function MeasureProlygon(props) {
  ...
  let formatArea = (polygon) => {
    ...
    if (area > 10000) {
      output =
        ...
        " " +
        "ตร.กม." +
        calrai(Math.round(area * 100) / 100);
    } else {
      output =
        "พื้นที่ขอเช่า " +
  ...
  function calrai(x) {
    var raidd = x / 1600;
    var rai = parseInt(raidd);
    var ngan = parseInt((raidd - rai) * 4);
    var wah = Math.round(((raidd - parseInt(raidd)) * 4 - ngan) * 100, 2);
    return rai + " ไร่ " + ngan + " งาน " + wah + " ตร.ว.";
  }
  ...
  const addInteraction = () => {
    if (props.map != undefined) {
      ...
      const activeTip =
        "ลากเส้นต่อไปเพื่อวัดพื้นที่";
      const idleTip = "คลิกเพื่อเริ่มการวัดพื้นที่";
      let tip = idleTip;
      draw = new Draw({
        source: source,
        type: drawType,
