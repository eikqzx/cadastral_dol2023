...
export default function SideTreeView(props) {
    ...
    const onCreateObject = async () => {
        ...
        if (onSearchDataFiltered.length != 0) {
            onSearchDataFiltered.sort((a, b) => a.PRINTPLATE_TYPE_SEQ != 10 ? a.PARCEL_LAND_SURVEY_NO - b.PARCEL_LAND_SURVEY_NO : a.CONDOROOM_HSFS_SEQ - b.CONDOROOM_HSFS_SEQ);
        ...
        // if (processSeq == 103) {
        //     for (const key in onSearchData) {
        //         let parcelHSFSGroupData = []
        //         if (Object.hasOwnProperty.call(onSearchData, key)) {
        //             const element0 = onSearchData[key];
        //             // console.log(element,"onSearchData");
        //             let resIndex = await getRegParcelIndexByParcelSeq(element0.PARCEL_SEQ);
        //             console.log(resIndex, "onSearchData resIndex");
        //             for (const key1 in resIndex) {
        //                 if (Object.hasOwnProperty.call(resIndex, key1)) {
        //                     const element1 = resIndex[key1];
        //                     let objGroup = {
        //                         PARCEL_INDEX_SEQ: element1.PARCEL_INDEX_SEQ,
        //                         PROCESS_SEQ_: Number(processSeq)
        //                     }
        //                     let resParcelHSFSGroup = await parcelHSFSByParcelIndexSeqGroup(objGroup);
        //                     console.log(resParcelHSFSGroup, "onSearchData resParcelHSFSGroup");
        //                     for (const key in resParcelHSFSGroup) {
        //                         if (Object.hasOwnProperty.call(resParcelHSFSGroup, key)) {
        //                             const element2 = resParcelHSFSGroup[key];
        //                             element2.PARCEL_SEQ = element1.PARCEL_SEQ
        //                             element2.PARCEL_SURVEY_NO = element0.PARCEL_SURVEY_NO
        //                             parcelHSFSGroupData.push(element2)
        //                         }
        //                     }
        //                 }
        //             }
        //             let childData = parcelHSFSGroupData.filter(item => item.PARCEL_SEQ == element0.PARCEL_SEQ)
        //             element0.DATA = childData;
        //         }
        //     }
        //     let statusSeq101 = onSearchData.filter(item => item.DATA.some(item => item.STATUS_SEQ == 101));
        //     let statusSeq102 = onSearchData.filter(item => item.DATA.some(item => item.STATUS_SEQ == 102));
        //     let statusSeq103 = onSearchData.filter(item => item.DATA.some(item => item.STATUS_SEQ == 103));
        //     let statusSeq104 = onSearchData.filter(item => item.DATA.some(item => item.STATUS_SEQ == 104));
        //     console.log(statusSeq104, "statusSeq104");
        //     masStatus.map(item => {
        //         item.DATA = [];
        //         if (item.STATUS_SEQ == 101) {
        //             item.DATA = statusSeq101
        ...
        //     console.log(masStatus, "masStatus");
        //     await setTreeViewData(masStatus.sort((a, b) => b.STATUS_SEQ - a.STATUS_SEQ));
        //     // console.log(parcelHSFSGroupData,"parcelHSFSGroupData");
        // }
        // if (processSeq == 106) {
        //     for (const key in onSearchData) {
        //         let parcelHSFSGroupData = []
